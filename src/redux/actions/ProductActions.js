import {GET_PRODUCTS, SET_LOADING_PRODUCT, DATA_ON_SEARCH} from './type';
...
export const getProductsHome = () => async dispatch => {
  try {
    ...
    const result = await axios.get(
      API.API_URL.concat(
        'product/all?search[key]=products.name&search[value]=&limit=15sort[key]=products.id&sort[value]=1',
      ),
    );
    if (result.data.success) {
      dispatch({
